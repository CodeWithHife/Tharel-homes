  ...
  return (
    <>
      <style>{`
        ...
        .db-property-img { width: 100%; height: 160px; object-fit: cover; }
        ...
        .db-prop-btn { padding: 6px 12px; border-radius: 8px; border: 1.5px solid #E2E8F0; background: #fff; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 4px; }
        ...
        .modal-content { background: #fff; border-radius: 20px; padding: 32px 24px; max-width: 560px; width: 100%; max-height: 90vh; overflow-y: auto; }
      ...
      <div className="db-page">
        ...
        <div className="db-body">
          ...
          <main className="db-main">
            {tab === "overview" && (
              <div>
                <div className="db-welcome"><h1>Welcome back, {user.firstName}! 👋</h1><p>Here's your realtor dashboard overview.</p></div>
                ...
                <div style={{background:"#fff",borderRadius:"16px",padding:"20px 24px",border:"1px solid #E2E8F0",marginBottom:"24px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:"8px"}}>
                    <div><div style={{fontSize:"13px",color:"#64748b"}}>Listings Used</div><div style={{fontSize:"20px",fontWeight:800,color:"#0F172A"}}>{usedListings} / {typeof remainingListings === "string" ? remainingListings : planListings}</div></div>
                    <div style={{fontSize:"13px",color:"#64748b"}}>Plan: <strong style={{color:"#D4A017"}}>{activePlan.name}</strong></div>
                  </div>
                  <div style={{height:"6px",background:"#E2E8F0",borderRadius:"999px",marginTop:"12px",overflow:"hidden"}}>
                    <div style={{height:"100%",width: planListings === Infinity ? "30%" : Math.min(100, (usedListings / planListings) * 100) + "%",background: usedListings >= planListings ? "#ef4444" : "linear-gradient(90deg, #D4A017, #b8860c)",borderRadius:"999px",transition:"width 0.4s ease"}} />
                  </div>
                  {usedListings >= planListings && planListings !== Infinity && <p style={{fontSize:"12px",color:"#ef4444",marginTop:"8px"}}>You've reached your listing limit. Upgrade your plan to add more.</p>}
                </div>
                <div className="db-section-header"><div className="db-section-title">Recent Listings</div><button className="db-add-btn" onClick={() => setShowAddModal(true)}><Plus size={16} /> Add Property</button></div>
                {properties.length === 0 ? (
                  <div style={{padding:"40px 20px",background:"#fff",borderRadius:"16px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                    <p style={{color:"#94A3B8"}}>You haven't listed any properties yet.</p>
                    <button className="db-add-btn" style={{marginTop:"12px",display:"inline-flex"}} onClick={() => setShowAddModal(true)}><Plus size={16} /> Add Your First Property</button>
                  </div>
                ) : (
                ...
                {properties.length > 3 && <div style={{textAlign:"center",marginTop:"16px"}}><button className="db-add-btn" style={{background:"transparent",color:"#D4A017",border:"2px solid #D4A017"}} onClick={() => setTab("properties")}>View All ({properties.length})</button></div>}
              </div>
            )}

            {tab === "properties" && (
              <div>
                <div className="db-section-header"><div className="db-welcome" style={{marginBottom:0}}><h1>My Properties</h1><p>Manage all your property listings.</p></div><button className="db-add-btn" onClick={() => setShowAddModal(true)}><Plus size={16} /> Add Property</button></div>
                {properties.length === 0 ? (
                  <div style={{padding:"60px 20px",background:"#fff",borderRadius:"16px",border:"1px solid #E2E8F0",textAlign:"center"}}>
                    <p style={{color:"#94A3B8"}}>You haven't listed any properties yet.</p>
                    <button className="db-add-btn" style={{marginTop:"12px",display:"inline-flex"}} onClick={() => setShowAddModal(true)}><Plus size={16} /> Add Your First Property</button>
                  </div>
                ) : (
      ...
      {(showAddModal || editingProperty) && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) { setShowAddModal(false); setEditingProperty(null); resetForm(); } }}>
          <div className="modal-content">
            <div className="modal-header"><h2>{editingProperty ? "Edit Property" : "Add New Property"}</h2><button className="modal-close" onClick={() => { setShowAddModal(false); setEditingProperty(null); resetForm(); }}><X size={24} /></button></div>
            ...
            <div className="form-row"><div className="form-group"><label className="form-label">Price *</label><input className="form-input" type="number" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} placeholder="150000000" /></div><div className="form-group"><label className="form-label">Price Label *</label><input className="form-input" value={formData.priceLabel} onChange={e => setFormData({...formData, priceLabel: e.target.value})} placeholder="₦150,000,000" /></div></div>
            ...
            <div className="form-row"><div className="form-group"><label className="form-label">Beds</label><input className="form-input" type="number" value={formData.beds} onChange={e => setFormData({...formData, beds: e.target.value})} placeholder="3" /></div><div className="form-group"><label className="form-label">Baths</label><input className="form-input" type="number" value={formData.baths} onChange={e => setFormData({...formData, baths: e.target.value})} placeholder="2" /></div></div>
            ...
            <button className="form-submit" onClick={editingProperty ? handleEditProperty : handleAddProperty}>{editingProperty ? "Update Property" : "Add Property"}</button>
          </div>
        </div>
      )}
    </>
